...
def do_fda(im_src, tarimgpath):
    # im_src = Image.open(srcimgpath).convert('RGB')
    im_trg = Image.open(tarimgpath).convert('RGB')
    ...
    src_in_trg = FDA_source_to_target_np( im_src, im_trg, L=0.002 )

    src_in_trg = src_in_trg.transpose((1,2,0))
    # print('converting: ', srcimg, 'target: ', srcimg, "("+str(num_source_img)+")")
    toimage(src_in_trg, cmin=0.0, cmax=255.0).save(tarimgpath)